...
# Rigor Keywords: Looking for evidence of self-correction
KEYWORDS = ['wait', 'mispriced', 'slippage']

def run_rigorous_test():
    test_file = "finetune/traps_test.jsonl"
    if not os.path.exists(test_file):
        print(f"Error: {test_file} not found. Run 'python3 finetune/generate_data.py' first.")
        return

    # Load the model with adapters via PolyEngine
    engine = PolyEngine()
    ...
    with open(test_file, "r") as f:
        for line in f:
            ...
            # The structure in traps_test.jsonl is based on OpenAI/Qwen ChatML
            # We extract the user message content
            user_msg = data["messages"][1]["content"][0]["text"]
            
            try:
                # Parsing components from the generated text
                # Format: "Analyze this {category} scenario: {market}. Odds: {odds}. News: {news}. Initial Thought: {initial_thought}"
                ...
                # The Rigor: Check for <|think|> tag AND specific correction keywords
